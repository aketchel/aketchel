##  Arthur Ketchel II 👋

Seasoned leader with 22+ years of experience bridging the gap between complex engineering and business value. A full-stack engineer at heart, I specialize in architecting modern SaaS platforms from the ground up and scaling engineering organizations from lean startups to teams of 80+.

- 🔭 I'm currently working on architecting a suite of next-gen SaaS tools utilizing autonomous coding agents and agentic secure software development lifecycle workflows.
- 🏗️ In Development:
        * A streamlined AI-powered HOA management platform designed to simplify community governance and financial transparency.
        * An AI-powered career assistant that optimizes job transitions and professional growth through intelligent career path guidance and automation.
        * A high-performance travel analytics engine built to provide real-time clarity across complex pricing ecosystems.
        * A specialized tool for identifying and evaluating emerging opportunities in the real estate and tech markets.
- 🌱 Exploring: The intersection of AI and productivity—specifically autonomous coding agents (OpenClaw, Claude Code), MCP, and local LLM orchestration.
- 👯 Collaborating: Always open to discussing open-source contributions, SaaS products, entrepenurial ventures or advising innovative startups - especially tech focused startups.
- 🤔 I'm looking for help with **connecting with like-minded entrepreneurs, investors, and software engineers**.
- 💬 Ask me about: IT consulting, software engineering, technology leadership, cloud architecture, agile delivery, real estate, scaling software development teams and entrepreneurship.
- 📫 How to reach me: (803)524-2802 (cell) · aketchel@proinstincts.com (email)
- 😄 Pronouns: He / Him
- ⚡ Fun fact: I've led engineering teams across industries including fintech, nonprofit tech, freight, and insurance. Beyond the IDE, I hold real estate licenses in both **North Carolina and South Carolina** and an insurance license in ** South Carolina ** and am an avid supporter of nonprofits including but not limited to volunteer emergency (911) rescue organizations and local youth scouting leadership.

---

### 🧰 Skills & Expertise

**Leadership & Delivery**
- Engineering team leadership (5–80+ engineers), veteran of agile/SAFe and "Extreme Ownership" frameworks.
- Operations: Application Lifecycle Management (ALM), DevOps maturity, CI/CD strategy, post-deployment support/monitoring, performance tuning and incident management.
- Mentorship: Developing career paths for junior engineers, senior staff engineers and architects through engineering managers and directors.

**Architecture & Engineering**
- Full-stack: C# / .NET, Java, Python, React, Node.js, TypeScript, PHP, VB.NET, etc.
- Cloud: AWS (EC2, ECS, RDS, Aurora, Lambda, CloudWatch), Azure
- Databases: SQL Server, MySQL, PostgreSQL, MongoDB, Oracle, DB2
- Patterns: Multi-tenant SaaS, Microservices, Event-Driven Design, RESTful APIs.

**AI, ML & Future Tech**

- Tools: GitHub Copilot, OpenClaw, LangChain, Ollama, Agentic Workflows, autonomous coding agents, Machine Learning, etc.
- Emerging: Exploring Quantum Finance applications and machine learning for data analysis.

**DevOps, Observability & Tooling**
- CI/CD: GitHub Actions, Azure DevOps, CircleCI, TravisCI, TFS / Jenkins, Ansible
- Monitoring: New Relic, Cloudflare, Splunk, ElasticSearch, Grafana, Pendo
- ALM: Jira (Atlassian), Azure DevOps / TFS, Git (GitHub, Bitbucket, TFS)

**Other**
- Entrepreneurship — founded and ran Majistic Inc. (web hosting + café) for 8 years
- Real Estate — licensed Realtor in NC & SC (Fort Mill, SC area)
- B.S. Computer Science, University of South Carolina

---

### 💼 Career Highlights

| Period | Role | Company | Description
|---|---|---|---|
| Jun 2025 – Apr 2026 | Software Development Manager | Total Quality Logistics (TQL) | Modernizing freight tech
| May 2023 – Jan 2025 | Senior Director, Research and Development (R&D) [GiveSmart] | Community Brands / Momentive Software | Leading Fintech/Non-Profit Fundraising SaaS
| Sep 2022 – Apr 2023 | Software Delivery Manager (contract) | NFP Corp via PlanIT Group |
| Oct 2021 – Aug 2022 | Software Development Manager | NASCENT Technology | Automated Gate Systems, Logistics & Supply Chain Automation
| Jul 2018 – Oct 2021 | Vice President, App Dev Team Lead | Bank of America | Large-scale Financial Systems and Cross Entity Financial Analysis Monitoring
| Sep 2017 – Jul 2018 | Senior Consultant | Ettain Group (client: Bank of America) | Large-scale Financial Systems and Cross Entity Financial Analysis Monitoring
| Mar 2016 – Sep 2017 | Application Development Lead | Garretson Resolution Group | MassTort Case Management - Legal Technology
| Apr 2013 – Dec 2015 | Senior Consultant | CTS, Inc. | Consulting in multiple industries (Health Insurance, Nuclear Energy, Mortgage Insurance, etc.)
| Jan 2009 – Apr 2011 | Solutions Delivery / Technical Manager | Integration Point | Global Trade Software (ISF, FTZ, etc.)
| Jan 2002 – Feb 2010 | President & Founder | Majistic Inc. | Entrepreneurship & Managed Services & Quick Service Resturant

---

### 🌐 Find Me Online

[![Personal Website](https://img.shields.io/badge/Website-aketchel.com-blue?style=flat-square&logo=google-chrome)](http://www.aketchel.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-aketchel2-0A66C2?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/aketchel2)

